...
let adminPendingUsersState = null;
let tipstersListState = null;
let adminCreditsListState = { messageId: 0, chatId: 0 }; // null initially
let adminTipsListState = null;
...
if (!ADMIN_ID) {
    console.warn("ADMIN_TELEGRAM_ID not set - admin commands will not work correctly");
}
exports.bot = new node_telegram_bot_api_1.default(token, { polling: false });
// Global state for admin input
let adminCreditInput = null;
// Helper: upsert user on /start
async function registerUser(telegramId, username) {
    const existing = await db_1.pool.query(`SELECT id, telegram_id, username, role, status, requested_role
     FROM betting_bot.users
     WHERE telegram_id = $1`, [telegramId]);
    if (existing.rows.length > 0) {
        return existing.rows[0];
    }
    const insert = await db_1.pool.query(`INSERT INTO betting_bot.users (telegram_id, username, role, status, requested_role)
     VALUES ($1, $2, 'USER', 'PENDING', NULL)
     RETURNING id, telegram_id, username, role, status, requested_role`, [telegramId, username || null]);
    const user = insert.rows[0];
    await db_1.pool.query(`INSERT INTO betting_bot.wallets (user_id, balance, currency)
     VALUES ($1, 0, 'FAKE')`, [user.id]);
    return user;
}
exports.bot.onText(/\/start/, async (msg) => {
    ...
    if (!user.requested_role) {
        await exports.bot.sendMessage(chatId, "Welcome! Choose which *role* you want to sign up for:", {
            parse_mode: "Markdown",
            reply_markup: {
                inline_keyboard: [
                    [
...
`;
    ...
    await exports.bot.sendMessage(chatId, welcomeText);
});
exports.bot.onText(/balance/, async (msg) => {
    if (!msg.from)
        return;
    const chatId = msg.chat.id;
    const telegramId = msg.from.id;
  ...
  `, [telegramId]);
    if (userRes.rows.length === 0) {
        return exports.bot.sendMessage(chatId, "❌ You are not registered. Send /start first.");
    }
    const user = userRes.rows[0];
    if (user?.status !== "APPROVED") {
        return exports.bot.sendMessage(chatId, `⏳ Your account is **${user.status}**. Cannot check balance yet.`, { parse_mode: "Markdown" });
    }
    ...
    await exports.bot.sendMessage(chatId, message, { parse_mode: "Markdown" });
});
// 🔥 USERS: /tipsters - list sa AUTOMATSKIM AZURIRANJEM
exports.bot.onText(/\/tipsters/, async (msg) => {
    if (!msg.from)
        return;
    const chatId = msg.chat.id;
    const telegramId = msg.from.id;
    const userRes = await db_1.pool.query(`SELECT id, role, status FROM betting_bot.users WHERE telegram_id = $1`, [telegramId]);
    if (userRes.rows.length === 0) {
        return exports.bot.sendMessage(chatId, "You're not registered. Send /start first.");
    }
    const user = userRes.rows[0];
    if (user.status !== "APPROVED" || user.role === "TIPSTER") {
        return exports.bot.sendMessage(chatId, "You don't have access to this command.");
    }
...
async function renderTipstersList(chatId, userId, messageId) {
    // Dohvati tipstere i subscripcije
    const allTipsters = await db_1.pool.query(`SELECT id, username
     FROM betting_bot.users
     WHERE role = 'TIPSTER' AND status = 'APPROVED'
     ORDER BY username NULLS LAST, created_at ASC
     LIMIT 20`);
    const userSubscriptions = await db_1.pool.query(`SELECT ts.tipster_id
     FROM betting_bot.tipster_subscribers ts
     WHERE ts.user_id = $1`, [userId]);
    const subscribedTipsterIds = new Set(userSubscriptions.rows.map((row) => row.tipster_id));
    ...
    const messageText = `🎯 **Active tipsters (${totalTipsters}):**\n\n` +
        `📊 **Your subscriptions: ${subscriptionCount}/${totalTipsters}**\n\n`;
    const message = await exports.bot.sendMessage(chatId, messageText, {
        parse_mode: "Markdown",
        reply_markup: {
...
exports.bot.onText(/\/pending_users/, async (msg) => {
    if (msg.from?.id !== ADMIN_ID) {
        return exports.bot.sendMessage(msg.chat.id, "You have no rights for this command.");
    }
    const chatId = msg.chat.id;
    const result = await db_1.pool.query(`SELECT id, username, telegram_id
     FROM betting_bot.users
     WHERE status = 'PENDING'
     ORDER BY created_at ASC
     LIMIT 20`);
    if (result.rows.length === 0) {
        return exports.bot.sendMessage(chatId, "No users waiting for approval.");
    }
    ...
    const message = await exports.bot.sendMessage(chatId, `⏳ **Pending users (${result.rows.length}):**`, {
        parse_mode: "Markdown",
        reply_markup: { inline_keyboard: keyboard },
    });
    adminPendingUsersState.messageId = message.message_id;
});
exports.bot.onText(/pending_tips/, async (msg) => {
    if (msg.from?.id !== ADMIN_ID)
        return exports.bot.sendMessage(msg.chat.id, "No rights.");
    const chatId = msg.chat.id;
    const tips = await db_1.pool.query(`
  ...
  `);
    if (tips.rows.length === 0)
        return exports.bot.sendMessage(chatId, "No open tips.");
    const keyboard = [];
    ...
    const message = await exports.bot.sendMessage(chatId, `*OPEN TIPS* (${tips.rows.length})`, {
        parse_mode: "Markdown",
        reply_markup: { inline_keyboard: keyboard },
    });
    adminTipsListState = { messageId: message.message_id, chatId };
});
exports.bot.onText(/newtip (.+)/, async (msg, match) => {
    if (!msg.from)
        return;
    const chatId = msg.chat.id;
    const telegramId = msg.from.id;
    const userRes = await db_1.pool.query("SELECT id, role, status FROM bettingbot.users WHERE telegramid = $1", [telegramId]);
    if (userRes.rows.length === 0) {
        return exports.bot.sendMessage(chatId, "You are not registered in the system.");
    }
    const user = userRes.rows[0];
    if (user.status !== "APPROVED" || user.role !== "TIPSTER") {
        return exports.bot.sendMessage(chatId, "You do not have permission to publish tips.");
    }
    const payload = match?.[1];
    if (!payload) {
        return exports.bot.sendMessage(chatId, "Wrong format. Use: newtip BookingCode odds stake");
    }
    const parts = payload.split(" ").map(p => p.trim()).filter(p => p.length > 0);
    if (parts.length !== 3) {
        return exports.bot.sendMessage(chatId, "Wrong format. You need 3 fields: BookingCode odds stake");
    }
    const [bookingCode, oddsStrRaw, stakeStrRaw] = parts;
    // Validacija: Booking code mora imati točno 7 znakova
    if (bookingCode?.length !== 7) {
        return exports.bot.sendMessage(chatId, "❌ Booking code must have exactly 7 characters.");
    }
    ...
    if (!Number.isFinite(odds) || !Number.isFinite(recommendedStake)) {
        return exports.bot.sendMessage(chatId, "❌ Odds and stake must be valid numbers (decimals allowed, e.g. 2.10, 100.50)");
    }
    if (odds <= 1 || recommendedStake <= 0) {
        return exports.bot.sendMessage(chatId, "❌ Odds must be > 1 and stake must be > 0.");
    }
    // Provjera balansa
    const tipsterWallet = await db_1.pool.query("SELECT balance FROM bettingbot.wallets WHERE userid = $1", [user.id]);
    if (!tipsterWallet.rows[0] ||
        Number(tipsterWallet.rows[0].balance) < recommendedStake * odds) {
        return exports.bot.sendMessage(chatId, "❌ You don't have enough balance to cover recommended stake.");
    }
    const insert = await db_1.pool.query(`INSERT INTO bettingbot.tips (tipsterid, bookingcode, odds, recommendedstake, status)
     VALUES ($1, $2, $3, $4, 'OPEN')
     RETURNING id`, [user.id, bookingCode, odds, recommendedStake]);
    const tipId = insert.rows[0].id;
    await exports.bot.sendMessage(chatId, `✅ Tip ${tipId} created!\n📝 Booking: \`${bookingCode}\`\n🎯 Odds: ${odds}\n💰 Recommended stake: ${recommendedStake}`, { parse_mode: "Markdown" });
    // Send to subscribers (WHO HAVE ENOUGH BALANCE) - 3-min timeout notification
    const subs = await db_1.pool.query(`SELECT u.id, u.telegramid, w.balance 
     FROM bettingbot.tipstersubscribers ts 
     JOIN bettingbot.users u ON u.id = ts.userid 
     JOIN bettingbot.wallets w ON w.userid = u.id 
     WHERE ts.tipsterid = $1 AND u.status = 'APPROVED' AND w.balance >= $2`, [user.id, recommendedStake]);
    const tipText = `🆕 *New tip ${tipId}*\n📝 Booking Code: \`${bookingCode}\`\n🎯 Odds: ${odds}\n💰 Recommendation: ${recommendedStake}\n⏰ Expires in 3 minutes!`;
    for (const row of subs.rows) {
        const subChatId = row.telegramid;
        // Send notification immediately (3-min logic handled by frontend or manual close)
        try {
            await exports.bot.sendMessage(subChatId, tipText, {
                parse_mode: "Markdown",
                reply_markup: {
                    inline_keyboard: [
                        [
...
exports.bot.onText(/^\d+$/, async (msg) => {
    const chatId = msg.chat.id;
    const adminId = msg.from?.id;
    const inputAmount = Number(msg.text);
    if (!adminCreditInput)
        return;
    if (adminId !== adminCreditInput.adminId)
        return;
    if (inputAmount <= 0 ||
        inputAmount > 999999 ||
        !Number.isInteger(inputAmount)) {
        return exports.bot.sendMessage(chatId, "Enter **positive integer** 1-999999!", {
            parse_mode: "Markdown",
        });
    }
    ...
    `, [amount, targetUserId]);
        ...
        const transactionType = adminCreditInput.action === "ADD" ? "ADMINCREDIT" : "ADMINDEBIT";
    ...
    `, [targetUserId, amount, transactionType]);
        ...
        const userInfo = await db_1.pool.query(`SELECT username, telegram_id FROM betting_bot.users WHERE id = $1`, [targetUserId]);
        ...
        await exports.bot.sendMessage(chatId, `${inputAmount} credits ${actionText}! ${name}\n**New Balance:** ${newBalance.toFixed(2)}`, { parse_mode: "Markdown" });
        exports.bot
            .sendMessage(userData.telegramid, `${inputAmount} credits ${actionText}! New balance: ${newBalance.toFixed(2)}`, { parse_mode: "Markdown" })
            .catch(() => { });
        if (adminCreditInput.action === "REMOVE" && newBalance < 0) {
            await exports.bot.sendMessage(chatId, "⚠️ Balance went negative!");
        }
        // Vraća na pending users listu nakon unosa kredita
        if (adminPendingUsersState && adminPendingUsersState.chatId === chatId) {
            try {
                const result = await db_1.pool.query(`SELECT id, username, telegram_id FROM betting_bot.users WHERE status = 'PENDING' ORDER BY created_at ASC LIMIT 20`);
                let keyboard = [];
                for (const row of result.rows) {
                    const displayName = row.username ?? `ID ${row.telegram_id}`;
                    keyboard.push([
                ...
                await exports.bot.editMessageText(messageText, {
                    chat_id: chatId,
        ...
        await exports.bot.sendMessage(chatId, `❌ Failed: ${error.message}`);
    }
    finally {
...
exports.bot.onText(/\/help/, async (msg) => {
    ...
    const chatId = msg.chat.id;
    const telegramId = msg.from.id;
    const userRes = await db_1.pool.query(`SELECT id, role, status, username
       FROM betting_bot.users
       WHERE telegram_id = $1`, [telegramId]);
    if (userRes.rows.length === 0) {
        return exports.bot.sendMessage(chatId, "You're not registered. Send /start.");
    }
    const user = userRes.rows[0];
    ...
    await exports.bot.sendMessage(chatId, helpText);
});
exports.bot.onText(/credits/, async (msg) => {
    if (msg.from?.id !== ADMIN_ID) {
        return exports.bot.sendMessage(msg.chat.id, "No rights.");
    }
    const chatId = msg.chat.id;
    const usersRes = await db_1.pool.query(`
    ...
    LIMIT 20
  `);
    const tipstersRes = await db_1.pool.query(`
    SELECT id, username, telegram_id, role 
    FROM betting_bot.users 
  ...
  `);
    if (usersRes.rows.length === 0 && tipstersRes.rows.length === 0) {
        return exports.bot.sendMessage(chatId, "No approved users or tipsters found.");
    }
...
async function renderCreditsList(chatId) {
    const usersRes = await db_1.pool.query(`
    SELECT id, username, telegram_id, role 
    FROM betting_bot.users 
    WHERE status = 'APPROVED' AND role = 'USER' 
...
exports.bot.on("callback_query", async (query) => {
    ...
    if (data.startsWith("REQ_ROLE:")) {
        ...
        if (requestedRole !== "USER" && requestedRole !== "TIPSTER") {
            return exports.bot.answerCallbackQuery(query.id, {
                text: "Unknown role.",
                show_alert: true,
            });
        }
        // Updajtaj usera
        const res = await db_1.pool.query(`UPDATE betting_bot.users
       SET requested_role = $1, updated_at = NOW()
       WHERE telegram_id = $2
       RETURNING status`, [requestedRole, from.id]);
        ...
        await exports.bot.editMessageText(`✅ You signed up as *${requestedRole}*.\n\n` +
            (status === "APPROVED"
    ...
    if (data.startsWith("SUBSCRIBE:")) {
        ...
        const userRes = await db_1.pool.query(`SELECT id FROM betting_bot.users WHERE telegram_id = $1 AND status = 'APPROVED' AND role != 'TIPSTER'`, [from.id]);
        if (userRes.rows.length === 0) {
            return exports.bot.answerCallbackQuery(query.id, {
        ...
        await db_1.pool.query(`INSERT INTO betting_bot.tipster_subscribers (tipster_id, user_id)
     VALUES ($1, $2) ON CONFLICT (tipster_id, user_id) DO NOTHING`, [tipsterId, userRes.rows[0].id]);
        const tipster = await db_1.pool.query(`SELECT username FROM betting_bot.users WHERE id = $1`, [tipsterId]);
        ...
        if (tipstersListState && tipstersListState.userId === userRes.rows[0].id) {
            await renderTipstersList(tipstersListState.chatId, tipstersListState.userId, tipstersListState.messageId);
        }
        return;
    }
    ...
    if (data.startsWith("UNSUBSCRIBE:")) {
        ...
        const deleteRes = await db_1.pool.query(`DELETE FROM betting_bot.tipster_subscribers 
     WHERE tipster_id = $1 AND user_id = $2`, [tipsterId, userRes.rows[0].id]);
    ...
    if (data.startsWith("PENDING_SHOW:")) {
        ...
        if (from.id !== ADMIN_ID)
            return;
        const user = await db_1.pool.query(`SELECT id, username, telegram_id, created_at, requested_role
     FROM betting_bot.users 
     WHERE id = $1 AND status = 'PENDING'`, [userId]);
        ...
        await exports.bot.editMessageText(`👤 **${displayName}**\n` +
            `🆔 ID: ${userData.id}\n` +
        ...
        await exports.bot.answerCallbackQuery(query.id, { text: "User details" });
        return;
    }
    ...
    if (data.startsWith("APPROVE:")) {
        ...
        const userCheck = await db_1.pool.query(`SELECT id, username, telegram_id, role, status FROM betting_bot.users WHERE id = $1`, [userId]);
        ...
        await exports.bot.sendMessage(chatId, `✅ *Approval for ${user.username ?? user.telegram_id}*\n\nCurrent role: *${user.role}*\nTelegram ID: ${user.telegram_id}\n\nChoose **NEW role**:`, {
            parse_mode: "Markdown",
            reply_markup: {
                inline_keyboard: [
        ...
        await exports.bot.answerCallbackQuery(query.id, { text: "Choose role ➡️" });
        return;
    }
    // REJECT USER
    if (data.startsWith("REJECT:")) {
        ...
        if (from.id !== ADMIN_ID)
            return;
        const result = await db_1.pool.query(`UPDATE betting_bot.users
       SET status = 'REJECTED', updated_at = NOW()
       WHERE id = $1 RETURNING id, username, telegram_id`, [userId]);
        if (result.rows.length === 0) {
            return exports.bot.answerCallbackQuery(query.id, {
                text: "User doesn't exist.",
                show_alert: true,
        ...
        await exports.bot
            .sendMessage(user.telegram_id, "❌ Your account is rejected. Contact admin if you think it's a mistake.")
            .catch(() => { });
        await exports.bot.answerCallbackQuery(query.id, {
        ...
        if (adminPendingUsersState && adminPendingUsersState.chatId === chatId) {
         ...
         LIMIT 20`);
                if (result.rows.length > 0) {
                    ...
                    await exports.bot.sendMessage(chatId, `⏳ **Pending users (${result.rows.length}):**`, {
                        parse_mode: "Markdown",
                        reply_markup: { inline_keyboard: keyboard },
                    });
    ...
    if (data.startsWith("TIPSHOW")) {
        ...
        if (from.id !== ADMIN_ID)
            return;
        const tipRes = await db_1.pool.query(`
  ...
  `, [tipId]);
        if (tipRes.rows.length === 0) {
            exports.bot.answerCallbackQuery(query.id, {
                text: "Tip not found or already closed.",
                show_alert: true,
            });
        }
        const t = tipRes.rows[0];
        await exports.bot.editMessageText(`*Tip ${t.id}*\n` +
  ...
  `);
        if (tips.rows.length === 0) {
            await exports.bot.editMessageText("No open tips.", {
        ...
        await exports.bot.editMessageText(`*OPEN TIPS* (${tips.rows.length})`, {
            chat_id: adminTipsListState.chatId,
            message_id: adminTipsListState.messageId,
            parse_mode: "Markdown",
            reply_markup: { inline_keyboard: keyboard },
    ...
    `, [tipId]);
            if (tipRes.rows.length === 0) {
                await db_1.pool.query("ROLLBACK");
                return exports.bot.answerCallbackQuery(query.id, { text: "Tip not OPEN." });
            }
            const row = tipRes.rows[0];
            const tipsterid = row.tipster_id; // ✅ direktno
            const odds = row.odds;
            // Dohvati confirmed betove
            const userTips = await db_1.pool.query(`
    ...
    `, [result, tipId]);
            ...
            if (result === "WIN") {
                ...
                for (const ut of userTips.rows) {
                    ...
                    const userBalanceRes = await db_1.pool.query(`SELECT balance FROM betting_bot.wallets WHERE user_id = $1`, [ut.user_id]);
                    await exports.bot
                        .sendMessage(ut.telegram_id, `🎉 **TIP #${tipId} WON** ✅\n` +
                        `Stake: **${stake * odds}**\n` +
                        `💰 **Your balance: ${userBalanceRes.rows[0]?.balance}**`, { parse_mode: "Markdown" })
                        .catch(() => { });
                }
                // TIPSTER dobiva sve
                console.log(`Users: ${userTips.rows.length}, Total payout: ${totalTipsterPayout}, tipster: ${tipsterid}`);
                await db_1.pool.query(`
        UPDATE betting_bot.wallets SET balance = balance + $1 
        WHERE user_id = $2
      `, [totalTipsterPayout, tipsterid]);
                const tipsterBalanceRes = await db_1.pool.query(`SELECT balance, telegram_id FROM betting_bot.users u 
     JOIN betting_bot.wallets w ON w.user_id = u.id 
     WHERE u.id = $1`, [tipsterid]);
                ...
                if (tipsterTelegramId) {
                    await exports.bot
                        .sendMessage(tipsterTelegramId, `💰 **WIN BONUS!** 🎉\n` +
                        `Tip #${tipId}: **+${totalTipsterPayout}**\n` +
                        `(${userTips.rows.length} users)\n` +
                        `💵 **New balance: ${tipsterNewBalance}**`, { parse_mode: "Markdown" })
                        .catch(() => { });
                }
            }
            else if (result === "LOSE") {
        ...
        `, [userRefund, ut.user_id]);
                    const userBalanceRes = await db_1.pool.query(`SELECT balance FROM betting_bot.wallets WHERE user_id = $1`, [ut.user_id]);
                    await exports.bot
                        .sendMessage(ut.telegram_id, `😞 **TIP #${tipId} LOST**\n` +
                        `Stake: **${stake * odds}**\n` +
                        `Compensation: **+${userRefund}**\n` +
                        `💰 **New balance: ${userBalanceRes.rows[0]?.balance}**`, { parse_mode: "Markdown" })
                        .catch(() => { });
                }
                const tipsterInfo = await db_1.pool.query(`SELECT telegram_id FROM betting_bot.users WHERE id = $1`, [tipsterid]);
                if (tipsterInfo.rows[0]?.telegram_id) {
                    await exports.bot
                        .sendMessage(tipsterInfo.rows[0].telegram_id, `⚠️ **TIP #${tipId} LOST** (${userTips.rows.length} users)\n` +
                        `💰 **Your balance: unchanged**`, { parse_mode: "Markdown" })
                        .catch(() => { });
                }
            }
            await db_1.pool.query(`
    ...
    `);
            ...
            exports.bot.answerCallbackQuery(query.id, { text: "Error!", show_alert: true });
        }
    }
    ...
    if (data.startsWith("SET_ROLE:")) {
     ...
     RETURNING id, username, telegram_id, role`, [newRole, userId]);
        if (result.rows.length === 0) {
            return exports.bot.answerCallbackQuery(query.id, {
        ...
        try {
            await exports.bot.sendMessage(updatedUser.telegram_id, `🎉 *Your account is APPROVED!*\n\n` +
                `✅ Role: **${newRole}**\n` +
                `💰 Credits will be added soon...\n\n` +
                `Send /start or /help to see your commands.`, { parse_mode: "Markdown" });
        }
        catch (e) {
        ...
        await exports.bot.sendMessage(chatId, `✅ *${updatedUser.username ?? updatedUser.telegram_id}* → **${newRole}**\n\n` +
            `💰 **Enter credit amount:**\n` +
  ...
  `, [userId]);
        ...
        const user = userRes.rows[0];
        const name = user.username ?? `ID ${user.id}`;
        await exports.bot.editMessageText(`*${name}* (${user.role})\n\nChoose action:`, {
            chat_id: chatId,
            message_id: query.message?.message_id,
            parse_mode: "Markdown",
            reply_markup: {
        ...
        await exports.bot.answerCallbackQuery(query.id, { text: `Selected ${name}` });
        return;
    }
    // CREDITSADD/REMOVE callbacks
    if (data.startsWith("CREDITSADD:")) {
        const userId = Number(data.split(":")[1]);
        const userRes = await db_1.pool.query(`SELECT username, telegram_id FROM betting_bot.users WHERE id = $1`, [userId]);
        if (userRes.rows.length === 0)
            return exports.bot.answerCallbackQuery(query.id, {
                text: "User not found.",
                show_alert: true,
        ...
        await exports.bot.editMessageText(`*${name}*\nEnter amount to **ADD** (e.g. 1000):`, {
            chat_id: chatId,
            message_id: query.message?.message_id,
            parse_mode: "Markdown",
        });
        return exports.bot.answerCallbackQuery(query.id, { text: "Enter amount to ADD." });
    }
    ...
    if (data === "CREDITSBACK") {
        ...
        const { messageText, keyboard } = await renderCreditsList(adminCreditsListState.chatId);
        await exports.bot.editMessageText(messageText, {
        ...
        await exports.bot.answerCallbackQuery(query.id, { text: "Back to list." });
        return;
    }
    // 🔥 ADMIN: /pending_users - list with navigation state
    exports.bot.onText(/\/pending_users/, async (msg) => {
    ...
    if (data.startsWith("CONFIRM_BET:")) {
        ...
        const userRes = await db_1.pool.query(`SELECT u.id, w.balance 
       FROM betting_bot.users u 
       JOIN betting_bot.wallets w ON w.user_id = u.id 
       WHERE u.telegram_id = $1 AND u.status = 'APPROVED'`, [from.id]);
        const existingBet = await db_1.pool.query(`SELECT id FROM betting_bot.user_tips WHERE user_id = $1 AND tip_id = $2 AND status = 'CONFIRMED'`, [userRes.rows[0].id, tipId]);
        ...
        const tipRes = await db_1.pool.query(`SELECT tipster_id, odds, status, recommended_stake 
       FROM betting_bot.tips 
       WHERE id = $1 AND status = 'OPEN'`, [tipId]);
        if (tipRes.rows.length === 0) {
            return exports.bot.answerCallbackQuery(query.id, {
        ...
        const tipsterWallet = await db_1.pool.query(`SELECT balance FROM betting_bot.wallets WHERE user_id = $1`, [tipsterId]);
        ...
        try {
            ...
            await db_1.pool.query(`UPDATE betting_bot.wallets SET balance = balance - $1 WHERE user_id = $2`, [stake * odds, userId]);
            // Tipster pays stake upfront (will get back on WIN)
            await db_1.pool.query(`UPDATE betting_bot.wallets SET balance = balance - $1 WHERE user_id = $2`, [stake * odds, tipsterId]);
            // Record confirmed bet
            await db_1.pool.query(`INSERT INTO betting_bot.user_tips (user_id, tip_id, stake, status, potential_win)
         VALUES ($1, $2, $3, 'CONFIRMED', $4)`, [userId, tipId, stake, stake * odds]);
            await db_1.pool.query("COMMIT");
            await exports.bot.answerCallbackQuery(query.id, {
                text: `✅ Bet ${stake} recorded!`,
            });
            await exports.bot.sendMessage(chatId, `✅ Tip #${tipId} played with stake ${stake}.\nBalance updated!`);
            await exports.bot.sendMessage(chatId, `Tip ${tipId} - PLAYED! (you can't play again)`, {
                parse_mode: "Markdown",
                reply_markup: {
                    inline_keyboard: [
                        [
    ...
    if (data.startsWith("SKIP_BET:")) {
        await exports.bot.answerCallbackQuery(query.id, { text: "OK, skipped." });
        return;
    }
    if (data === "PENDING_BACK") {
        if (!adminPendingUsersState || from.id !== ADMIN_ID) {
            return exports.bot.answerCallbackQuery(query.id, { text: "Session expired." });
        }
        const result = await db_1.pool.query(`SELECT id, username, telegram_id
     FROM betting_bot.users
     WHERE status = 'PENDING'
     ...
     LIMIT 20`);
        if (result.rows.length === 0) {
            return exports.bot.answerCallbackQuery(query.id, { text: "No pending users." });
        }
        const keyboard = [];
        for (const row of result.rows) {
            const displayName = row.username ?? `ID${row.telegram_id}`;
        ...
        await exports.bot.editMessageText(`⏳ **Pending users (${result.rows.length}):**`, {
